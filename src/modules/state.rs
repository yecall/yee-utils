...
fn get_storage(rpc: &str, storage_key: StorageKey, block_number: Option<u64>) -> Result<Option<Vec<u8>>, String> {
    let mut runtime = Runtime::new().expect("qed");

    let block_hash = match block_number {
        Some(block_number) => {
            let result = runtime.block_on(base::rpc_call::<_, Option<String>>(
                rpc,
                "chain_getBlockHash",
                &(block_number, ),
            ))?.result.ok_or("decode failed".to_string())?;
            result
        }
        None => None,
    };
